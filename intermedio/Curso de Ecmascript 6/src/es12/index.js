// Clase 14

const string = "JavaScript es maravilloso, con JavaScript puedo crear el futuro de la web";

const remplacedString = string.replace("JavaScript", "Python");
console.log(remplacedString);

const remplacedString2 = string.replaceAll("JavaScript", "Python");
console.log(remplacedString2);

//

class Message {
    #show(val) {
        console.log(val);
    }
    get #add(val) {
        //...
    }
}

const message = new Message();
message.show("Hola!");

//

const promise1 = new Promise((resolve, reject) => reject("1"));
const promise2 = new Promise((resolve, reject) => resolve("2"));
const promise3 = new Promise((resolve, reject) => resolve("3"));

Promise.any([promise1, promise2, promise3]) /* resuelve el primero que tenga alcance, en este caso promise2 */
    .then(response => console.log(response));

//

class AnyClass {
    constructor(element) {
        this.ref = new WeakRef(element)
    }
    // {...}
}

//

let isTrue = true;
let isFalse = false;
console.log(isTrue &&= isFalse);

let isTrue1 = true;
let isFalse1 = false;
console.log(isTrue1 ||= isFalse1);

let isTrue2 = undefined;
let isFalse2 = false;
console.log(isTrue2 ??= isFalse2);